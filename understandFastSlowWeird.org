...
parsed using =parseFloat=. The string values in the last column
however will be added as is. Thus, the main question is whether the
space ends up in the "type" column. Let's write a short test for it.
...
After spending quite a bit more time on how to best handle the
stripping of whitespace, I finally ended up with the following
solution: The whitespace is stripped in =readCsv= (I experimented with
doing it in =toDf= instead). The =rowEntry= CSV proc actually returns
a =var string= which allows us to strip each entry in place before
assigning it. This finally gets rid of our copying bottleneck!
